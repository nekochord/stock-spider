...
from stock_spider.entitys import DayPrice
from stock_spider.utils import numberutil


class DayAnnouncementSpider(scrapy.Spider):
    """
        獲取股票的日Ｋ線資料
        資料來自富邦
    """
    ...
    def createRequest(self, stockCode):
        url = 'https://fubon-ebrokerdj.fbs.com.tw/z/BCD/czkc1.djbcd?a={code}&b=D&c=2880&E=1&ver=5'
    ...
    def insert(self, code, day, opening_price, highest_price, lowest_price, closing_price, amount):
        insert = Insert('day_price', values={
            'code': code,
            'day': day,
    ...
    def selectExistDatesByCode(self, code):
        daySet = set()
        dayPrices = DayPrice.select(DayPrice.q.code == code)
        for dayPrice in dayPrices:
            dateStr = dayPrice.day.strftime("%Y/%m/%d")
            daySet.add(dateStr)
        return daySet
